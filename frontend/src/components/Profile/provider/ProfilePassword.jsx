...
function Updateinfo() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await authApi.put('/provider/info/password', password);
      setAlert({
      ...
      setAlert({
        ...
        description: error.response?.data?.message || "Something went wrong while updating password.",
        status: true,
        buttonText: "Ok",
        onClose: handleClose,
        cancel: handleClose
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="flex items-center justify-end bg-gray-50 px-4 py-3 text-end shadow sm:rounded-bl-md sm:rounded-br-md sm:px-6">
              <button
                ...
                disabled={
                  !password.currentPassword || !password.newPassword || !confirmPassword || loading
                }
              >
                Change Password
              </button>
